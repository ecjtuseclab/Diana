...
function ViewModel() {
    ...
    me.DeleteUrl = "../index.ashx?ctrl=workflow&action=DeleteForm";
    ...
    me.ProcessDesign = function (me) {
        ...
        window.location.href = "Index.ashx?ctrl=workflow&action=WorkflowVisualization&wfid=" + $ko.toJS(smodel).id;
    } 
}
function vmExtend() {
    var me = this;
    ...
    me.LoadModels = function (pageIndex) {
        ...
        $ace.get("../index.ashx?ctrl=workflow&action=showAll", params, function (result) {
...
function Dialog(vm) {
    ...
    me.OnSave = function () {
        ...
        //提交表单数据
        $ace.post("../index.ashx?ctrl=workflow&action=SubmitForm", model, function (result) {
            $ace.msg(result.Msg);
            me.Close();
            vm.LoadModels();
        });
