...
function ViewModel() {
    ...
    me.DeleteUrl = "../index.ashx?ctrl=group&action=DeleteForm";
...
function vmExtend() {
    ...
    me.LoadModels = function (pageIndex) {
        ...
        $ace.get("../index.ashx?ctrl=group&action=showAll", params, function (result) {
...
function Dialog(vm) {
    ...
    me.OnSave = function () {
        ...
        //提交表单数据
        $ace.post("../index.ashx?ctrl=group&action=SubmitForm", model, function (result) {
            $ace.msg(result.Msg);
            me.Close();
            vm.LoadModels();
        });
