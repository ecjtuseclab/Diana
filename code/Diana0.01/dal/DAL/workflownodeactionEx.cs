...
///模块和代码页功能描述：操作SQLServer数据库的工作流节点动作的方法
///最后修改时间：2018/1/26
/// </summary>
namespace Diana.dal
{
    public class workflownodeactionEx : RepositoryBase<workflownodeaction>, IworkflownodeactionEx
    {
        ...
        public List<action> getactionList()
        {
            try
            {
                List<action> actionlist = db.SqlQuery<action>("select * from action where actiontype=4 or (actiontype=2 and  id not in (select actionid from workflownodeaction))");
