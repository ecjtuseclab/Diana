...
///模块和代码页功能描述：操作SQLServer数据库的资源表的方法
///最后修改时间：2018/1/26
/// </summary>
namespace Diana.dal
{
    public class resourceEx : RepositoryBase<resource>, IresourceEx
    {
        ...
        public resource getResourcebycontrolername(string controlername, int resourceleval)
        {
            resource table = db.Queryable<resource>().Where(d => d.controlername == controlername).Where(c => c.resourceleval == resourceleval).FirstOrDefault();
            return table;
        }
